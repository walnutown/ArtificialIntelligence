...
public class Graph {
   ...
   /**
    * Remove the old node and its edges, add the node with new attributes
    */
   public void setNodeLocation(Node n){
      int ni = list.indexOf(n);
      if (ni < 0)
         throw new NullPointerException("Node not found");
      Node old = list.get(ni);
      old.setLocation(n.getLocation());
   }
   ...
   public ArrayList<Node> getNeighbor(Node n) {
      int ni = list.indexOf(n);
